...
```R
mypoints=SpatialPointsDataFrame(coords=coordinates,
                                data=df.cultivo[,c(1:3)], proj4string = myCRS)
```
...
Para construir el variograma, hacer la interpolación y calcular la semivarianza es mejor (y más práctico), extraer una muestra. Para el ejemplo, se seleccionará una muestra de tamaño 500:


```R
data <- mypoints[sample(500,replace=F),]
head(data@data)
```


<table>
<thead><tr><th></th><th scope=col>COD_VEREDA</th><th scope=col>P_S6P46</th><th scope=col>Freq</th></tr></thead>
<tbody>
	<tr><th scope=row>193</th><td>05044010   </td><td>00159201001</td><td> 3         </td></tr>
	<tr><th scope=row>400</th><td>05120022   </td><td>00159201001</td><td>46         </td></tr>
	<tr><th scope=row>337</th><td>05107007   </td><td>00159201001</td><td> 2         </td></tr>
	<tr><th scope=row>33</th><td>05002059   </td><td>00159201001</td><td> 1         </td></tr>
	<tr><th scope=row>364</th><td>05113017   </td><td>00159201001</td><td>15         </td></tr>
	<tr><th scope=row>270</th><td>05051058   </td><td>00159201001</td><td> 3         </td></tr>
</tbody>
</table>



Con la muestra, se construye el variograma con ayuda del comando `variogram` de la librería `gstat`:


```R
library(gstat)
plot(variogram(log(Freq)~1, locations=coordinates(data),
     data=data, cloud=T),pch=16, cex=1)
```
...
Ahora, una búsqueda direccional


```R
plot(variogram(log(Freq)~1,
    locations=coordinates(data), data=data, cutoff=.5 ,
    cloud=F),type="b", pch=16)
```
...
```R
v <- variogram(log(Freq) ~ 1, locations=coordinates(data), data)
```


```R
library(lattice)
print(xyplot(gamma ~ dist, v, ylim=c(0,max(v$gamma)+2*sd(v$gamma)),
    pch = 3, type = 'b', lwd = 2,
	panel = function(x, y, ...) {
        for (i in 1:100) {
        	data$random <- sample(data$Freq)
        	v <- variogram(log(random) ~ 1, locations=coordinates(data), data)
...
```R
grd <- Sobj_SpatialGrid(data,maxDim=200)$SG
...
```R
kr <- krige(log(Freq)~1, data, grd, model=v.fit)
spplot(kr,col.regions=rev(terrain.colors(100)), 
       names.attr=c("Predicciones","Varianza"), 
       main="Kriging Ordinario, Cantidad de Cultivos de Yuca (log)",pch=2,cex=2)
```
...
```R
# k=.2
idw.out <- idw(log(Freq)~1,data,grd,idp=.2)
spplot(idw.out[1],col.regions=rev(heat.colors(100)),
main="IDW Interpolación, Cantidad de Cultivos de Yuca (log)",sub="k = 1/5")
```
...
```R
# k=1
idw.out <- idw(log(Freq)~1,data,grd,idp=1)
spplot(idw.out[1],col.regions=rev(heat.colors(100)), 
main="IDW Interpolation, Cantidad de Cultivos de Yuca (log)",sub="k = 1")
```
...
idw.out <- idw(log(Freq)~1,data,grd,idp=1)
spplot(idw.out[1],col.regions=rev(heat.colors(100)), 
main="IDW Interpolation, Cantidad de Cultivos de Yuca (log)",sub="k = 5")
```
